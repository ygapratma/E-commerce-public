...
# Load the main_data.csv file using pandas from the local directory
df = pd.read_csv('all_data.csv', parse_dates=['order_purchase_timestamp', 'order_approved_at', 'order_delivered_carrier_date', 'order_delivered_customer_date', 'order_estimated_delivery_date'])
...
def main():
    ...
    selected_page = st.sidebar.radio('Go to', ['Home', 'Analysis', 'Visualization'])
    ...
    if selected_page == 'Home':
        ...
        st.write('Perform analysis here...')
...
time_to_solve = df['waktu_untuk_satu_order'].mean()
